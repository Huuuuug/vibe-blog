...
const ThemeToggle = dynamic(
  () => import("@/components/layout/theme-toggle/theme-toggle").then((mod) => mod.ThemeToggle),
  { ssr: false },
);
    ...
    <header className="sticky top-0 z-10 w-full">
      <div className="grid w-full gap-4 bg-[var(--card-strong)] px-5 py-[14px] backdrop-blur-[16px] sm:px-6 lg:grid-cols-[minmax(0,1fr)_auto_minmax(0,1fr)] lg:items-center lg:px-8">
        ...
        <div className="flex justify-start lg:justify-center">
          <RadioGroup items={navItems} value={currentValue} onChange={(value) => router.push(value ? `/${value}` : "/")} />
        </div>
